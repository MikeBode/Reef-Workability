...
class WeatherExtractor:
    ...
    def load_and_cache_dataset(self, nc_file_path, param_name):
        if nc_file_path is None or not os.path.exists(nc_file_path):
            return None
        ...
        if cache_key not in self.dataset_cache:
            try:
                print(f"Loading dataset: {os.path.basename(nc_file_path)}")
    ...
    def extract_batch_6_hours_mean(self, nc_file_path, date, coords_array, param_name):
        ds = self.load_and_cache_dataset(nc_file_path, param_name)
        if ds is None:
